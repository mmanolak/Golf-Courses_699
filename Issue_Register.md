...
*(2026-08-10, same day: **P6-01 addendum, +0 net** — the author's reported Table 1 "Runaway
argument… no output PDF" `pdflatex` failure traced to a *different* file than the canonical
generator: `Bulk/R/output/8.1_Table1_Acreage.tex` (legacy, non-cascade script, unescaped `%`,
input CSV no longer present to regenerate it), not `Final_Thesis_Figures/8.141_Table1_Acreage.tex`
(canonical, `Phase_6.R`-generated, `VERIFIED` compiles cleanly standalone). Four escaping variants
round-trip tested with real `pdflatex` runs, not inference, to confirm rather than guess. Same
...
**Addendum (2026-08-10) — the reported "fatal LaTeX error" traced to a different, stale file,
not the canonical generator.** The author reported Table 1 producing "Runaway argument…
Emergency stop… no output PDF" when compiled, attributing it to `Phase_6.R`'s footnote code
despite the source (line ~1580) looking correct. Tested directly with `pdflatex` rather than
inferred:

- `output/Final_Thesis_Figures/8.141_Table1_Acreage.tex` (the canonical, `Phase_6.R`-generated,
  manuscript-cited file) **compiles cleanly**, standalone, with just `booktabs` loaded. Confirmed
  by hexdump that line 13's `\%` is a genuine single backslash + percent, no encoding artifact.
- A **different file**, `Bulk/R/output/8.1_Table1_Acreage.tex` — generated by a separate,
  non-canonical legacy script, `Bulk/R/8_LaTeX_Tables.R` (documented in
  `01 - Phase6_Documentation.md` as its own "Script 8", distinct from the master cascade's
  Script 8 inside `Phase_6.R`) — **reproduces the exact reported error, verbatim**, including the
  identical "Runaway argument? … ! File ended while scanning use of `\multicolumn`" trace. Its
  footnote text is genuinely unescaped (`99% confidence...`, no backslash at all) — this file
  predates the escaping fix and was never regenerated by it, because it isn't part of the live
  cascade and its own input (`Bulk Tests/R/National_Acreage_Summary.csv`) no longer exists on
  `strix` to regenerate it from.
- Round-trip tested four variants against the canonical generator's exact logic to make this
  non-guesswork: (A) current production code (quadruple-backslash source,
  `footnote(escape=FALSE)`) — compiles; (B) double-backslash source with `escape=TRUE` — compiles
  but over-escapes to a literal `\textbackslash{}\%`, wrong output even though it doesn't crash;
  (C) double-backslash source with `escape=FALSE` (the pre-fix pattern, and what `Bulk/R/8_LaTeX_
  Tables.R` still had) — fails with the exact reported error; (D) raw LaTeX string appended after
  `kbl()` bypassing `footnote()` — inconclusive due to a bug in this test's own string escaping,
  not pursued further since (A) already resolves the question.

**Applied the same escaping fix to `Bulk/R/8_LaTeX_Tables.R`** (Table 1's and Table 2's footnote
text, same doubled-backslash pattern) for hygiene, even though it isn't part of the live cascade
— but its output file on disk **could not be regenerated** (missing input CSV) and remains stale
and broken as committed. If the author's actual manuscript build references
`Bulk/R/output/8.1_Table1_Acreage.tex` rather than `Final_Thesis_Figures/8.141_Table1_Acreage.tex`,
