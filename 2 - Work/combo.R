...
osm_acres_only <- df_osm %>%
  ...
  # --- THIS IS THE FIX ---
  # Group by each unique course
  group_by(course_name, latitude, longitude) %>%
  # Sum the acreage for any duplicates (combines separate polygons)
  summarise(acres_osm = sum(acres_osm, na.rm = TRUE), .groups = 'drop') %>%
  # If the sum is 0 because all values were NA, turn it back to NA
...
final_master <- combined_data %>%
  select(
    course_name, city, state, street_address, zip_code, 
    final_acreage, acreage_source, 
    longitude, latitude
  )
...
print(paste("Final row count:", nrow(final_master))) # This should now match your original count